...
def sequence_to_tensors(sequence, dictionary, max_length):
    """Convert a sequence of words to a tensor of word indices."""
    sen_rep = torch.FloatTensor(max_length, len(dictionary)).zero_()
    for i in range(len(sequence)):
        if i < max_length:
            for j in range(len(sequence[i])):
                sen_rep[i, sequence[i][j]] = 1
        else:
            break
    return sen_rep


def batch_to_tensor(batch, dictionary, max_query_length, max_doc_length):
    query_tensor = torch.LongTensor(len(batch), max_query_length, len(dictionary))
    query_clicks = torch.LongTensor(len(batch), len(batch[0].rel_docs), max_doc_length, len(dictionary))
    click_labels = torch.FloatTensor(len(batch), len(batch[0].rel_docs))
    for i in range(len(batch)):
        query_tensor[i] = sequence_to_tensors(batch[i].letter_n_grams, dictionary, max_query_length)
        # shuffle the list of relevant documents
        numpy.random.shuffle(batch[i].rel_docs)
        for j in range(len(batch[i].rel_docs)):
            query_clicks[i, j] = sequence_to_tensors(batch[i].rel_docs[j].letter_n_grams, dictionary, max_doc_length)
